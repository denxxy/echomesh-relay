...
pub const RELAY_AUTH_MAGIC: &[u8; 4] = b"EMAU";
pub const RELAY_AUTH_OK_MAGIC: &[u8; 4] = b"EMOK";
...
pub enum RouteResult { Delivered, RecipientOffline, Backpressure }

#[derive(Clone)]
struct RouteEntry { connection_id: u64, tx: mpsc::Sender<Frame> }
...
impl RelayRouter {
    ...
    pub async fn unregister(&self, route_id: RouteId, connection_id: u64) {
        ...
        if routes.get(&route_id).map(|entry| entry.connection_id == connection_id).unwrap_or(false) {
            routes.remove(&route_id);
        }
    }
    pub async fn route(&self, sender: RouteId, mut frame: Frame) -> RouteResult {
        let recipient = frame.session_id;
        let tx = { self.routes.read().await.get(&recipient).map(|entry| entry.tx.clone()) };
...
/// Authentication is deliberately carried only after the Noise handshake.
/// The pseudo-TLS cover therefore contains no reusable bearer credential.
pub fn relay_auth_frame(token: &[u8]) -> Option<Frame> {
    if token.is_empty() || token.len() > u16::MAX as usize { return None; }
    let mut payload = Vec::with_capacity(6 + token.len());
    payload.extend_from_slice(RELAY_AUTH_MAGIC);
    payload.extend_from_slice(&(token.len() as u16).to_be_bytes());
    payload.extend_from_slice(token);
    Frame::new(ROUTER_CONTROL_ID, [0u8; 8], Bytes::from(payload)).ok()
}

pub fn parse_relay_auth(frame: &Frame) -> Option<&[u8]> {
    if frame.session_id != ROUTER_CONTROL_ID || frame.payload.len() < 6 || &frame.payload[..4] != RELAY_AUTH_MAGIC {
        return None;
    }
    let len = u16::from_be_bytes([frame.payload[4], frame.payload[5]]) as usize;
    if len == 0 || frame.payload.len() != 6 + len { return None; }
    Some(&frame.payload[6..])
}

pub fn relay_auth_ack_frame() -> Frame {
    Frame::new(ROUTER_CONTROL_ID, [0u8; 8], Bytes::from_static(RELAY_AUTH_OK_MAGIC))
        .expect("relay auth acknowledgement always fits")
}
...
pub fn parse_registration(frame: &Frame) -> Option<RouteId> {
    if frame.session_id != ROUTER_CONTROL_ID || frame.payload.len() != ROUTE_REGISTRATION_LEN || &frame.payload[..4] != ROUTE_REGISTER_MAGIC {
        return None;
    }
    let mut route_id = [0u8; 16]; route_id.copy_from_slice(&frame.payload[4..20]);
    let mut peer_id = [0u8; 32]; peer_id.copy_from_slice(&frame.payload[20..52]);
    let digest = Sha256::digest(peer_id);
    if digest[..16] != route_id { return None; }
...
mod tests {
    ...
    fn signed_registration(seed: [u8; 32]) -> Frame {
        ...
        let digest = Sha256::digest(peer_id);
        let mut route = [0u8; 16]; route.copy_from_slice(&digest[..16]);
    ...
    fn relay_auth_round_trip() {
        let frame = relay_auth_frame(b"unique-relay-secret").unwrap();
        assert_eq!(parse_relay_auth(&frame), Some(&b"unique-relay-secret"[..]));
        assert_eq!(&relay_auth_ack_frame().payload[..], RELAY_AUTH_OK_MAGIC);
    }
    ...
    fn forged_route_registration_is_rejected() {
        ...
        let mut payload = frame.payload.to_vec(); payload[4] ^= 1; frame.payload = Bytes::from(payload);
        assert_eq!(parse_registration(&frame), None);
    }

    #[test]
    fn legacy_unsigned_registration_is_rejected() {
        let mut payload = Vec::new(); payload.extend_from_slice(b"EMR1"); payload.extend_from_slice(&[0x42; 16]);
    ...
    async fn route_rewrites_destination_to_sender() {
        let router = RelayRouter::new(); let sender = [0x11; 16]; let recipient = [0x22; 16];
        let (tx, mut rx) = mpsc::channel(1); router.register(recipient, tx).await;
        ...
        let delivered = rx.recv().await.unwrap(); assert_eq!(delivered.session_id, sender); assert_eq!(&delivered.payload[..], b"ciphertext");
    }

    #[tokio::test]
    async fn stale_unregister_does_not_remove_replacement() {
        let router = RelayRouter::new(); let route = [0x44; 16];
        let (tx1, _rx1) = mpsc::channel(1); let (tx2, _rx2) = mpsc::channel(1);
        let id1 = router.register(route, tx1).await; let _id2 = router.register(route, tx2).await;
        router.unregister(route, id1).await; assert_eq!(router.active_routes().await, 1);
    }
}

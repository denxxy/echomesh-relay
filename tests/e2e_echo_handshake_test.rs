...
use echomesh_relay::config::ECHO_SERVICE_PEER_ID;
...
const TEST_AUTH_TOKEN: &[u8] = b"echomesh-e2e-auth-token-v1";
...
async fn test_e2e_echo_handshake_and_routing() {
    ...
    // Use an explicit non-empty test credential. Production DEFAULT_SECRET_TOKEN
    // is intentionally empty so deployments never share a compiled-in secret.
    let config = ListenerConfig::new("127.0.0.1:0".parse().unwrap(), TEST_AUTH_TOKEN.to_vec())
    ...
    let tls_builder = PseudoTlsBuilder::new(TEST_AUTH_TOKEN.to_vec(), "cloudflare.com");
    ...
    let noise_session = client_noise_handshake(&mut client_stream, &server_pub)
        .await
        .expect("Noise handshake must succeed with the explicit matching test token");
    ...
    // An unregistered connection cannot route arbitrary client-to-client data.
